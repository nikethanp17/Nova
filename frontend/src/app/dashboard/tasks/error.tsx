...
export default function TasksError({
  ...
  useEffect(() => {
    console.error("Tasks boundary caught error:", error);
  }, [error]);
    ...
    <div className="w-full min-h-[300px] flex items-center justify-center">
      <div className="glass p-6 rounded-2xl border border-red-500/20 bg-red-500/5 max-w-sm text-center flex flex-col items-center gap-3">
        ...
        <div className="space-y-1">
          <h3 className="text-white font-bold text-sm">Failed to Load Tasks</h3>
          <p className="text-xs text-slate-400 leading-relaxed">
            {error.message || "An error occurred while fetching tasks from database."}
          </p>
        </div>
        <button
          onClick={() => reset()}
        ...
        >
          <RotateCcw className="w-3.5 h-3.5" /> Retry Query
        </button>
      </div>
    </div>
  );
